...
import { version, repository, ConfigTS } from 'src/util/config';
import { BridgeMsg } from 'icg/transport/BridgeMsg';
import { file as fileTS } from 'icg/lib/handlers/Context';

const servemedia: ConfigTS[ 'transport' ][ 'servemedia' ] = Manager.config.transport.servemedia;

const USERAGENT = `AFC-ICG-BOT/${ version } (${ repository.url.replace( /^git\+/, '' ) })`;
 ...
 * 決定檔案去向
 */
async function uploadFile( file: fileTS ) {
	let url: unknown;
		...
		if ( context.extra.clients > 1 && context.extra.files && servemedia.type && servemedia.type !== 'none' ) {
			...
			for ( const [ index, file ] of context.extra.files.entries() ) {
				if ( servemedia.sizeLimit && servemedia.sizeLimit > 0 &&
					...
					logger.debug( `[file] <FileUploader> #${ context.msgId } File ${ index + 1 }/${ fileCount }: Size limit exceeded. Ignore.` );
				} else {
			...
			for ( const [ index, upload ] of uploads.entries() ) {
				logger.debug( `[file] <FileUploader> #${ context.msgId } File ${ index + 1 }/${ uploads.length } (${ upload.type }): ${ upload.url }` );
			}
...
bridge.addHook( 'bridge.send', async ( msg: BridgeMsg ) => {
	try {
		...
		logger.error( 'Error on processing files: ', e );
