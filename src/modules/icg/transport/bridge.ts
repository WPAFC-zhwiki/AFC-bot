...
import { Context, rawmsg } from 'icg/lib/handlers/Context';
...
export async function emitHook<V extends keyof hooks>( event: V, ...args: Parameters<hooks[V]> ): Promise<void> {
	...
	if ( hooks[ event ] ) {
		logger.debug( `[transport/bridge] emit hook: ${ event }` );
	...
	for ( const t of msg.extra.mapto ) {
		...
		if ( isbridge ) {
			promises.push( emitHook( 'bridge.receive', msg2 ).then( function () {
				...
				if ( processor ) {
					logger.debug( `[transport/bridge] <BotTransport> ${ msgid } ---> ${ new_uid.uid }` );
					return processor( msg2, { withNick, isNotice } );
				} else {
					logger.debug( `[transport/bridge] <BotTransport> ${ msgid } -X-> ${ new_uid.uid }: No processor` );
				}
			} ) );
		} else {
			// eslint-disable-next-line no-shadow
			const processor = processors.get( client );
			if ( processor ) {
				logger.debug( `[transport/bridge] <BotSend> ${ msgid } ---> ${ new_uid.uid }` );
				promises.push( processor( msg2, { withNick, isNotice } ) );
			} else {
				logger.debug( `[transport/bridge] <BotSend> ${ msgid } -X-> ${ new_uid.uid }: No processor` );
			}
		}
	}
...
export async function send( m: BridgeMsg | Context, bot?: symbol ): Promise<boolean> {
	...
	if ( bot === Manager.global.bot ) {
		...
		isbridge = false;
		logger.debug( `[transport/bridge] <BotSend> #${ currMsgId } ---> ${ msg.to_uid }: ${ msg.text }` );
		const extraJson: string = JSON.stringify( msg.extra );
		if ( extraJson !== 'null' && extraJson !== '{}' ) {
			logger.debug( `[transport/bridge] <BotSend> #${ currMsgId } extra: ${ extraJson }` );
		}
	} else {
		isbridge = true;
		logger.debug( `[transport/bridge] <UserSend> #${ currMsgId } ${ msg.from_uid } ---> ${ msg.to_uid }: ${ msg.text }` );
		const extraJson: string = JSON.stringify( msg.extra );
		if ( extraJson !== 'null' && extraJson !== '{}' ) {
			logger.debug( `[transport/bridge] <UserSend> #${ currMsgId } extra: ${ extraJson }` );
		}
	...
	try {
		...
		allresolved = false;
		logger.error( '[transport/bridge] <BotSend> Rejected: ', e );
	}

	emitHook( 'bridge.sent', msg );
	if ( promises.length > 0 ) {
		logger.debug( `[transport/bridge] <BotSend> #${ currMsgId } done.` );
	} else {
		logger.debug( `[transport/bridge] <BotSend> #${ currMsgId } has no targets. Ignored.` );
	}
