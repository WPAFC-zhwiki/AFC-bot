	...
	return function ( msg: BridgeMsg ) {
		if ( msg.command ) {
			...
			if ( disables.indexOf( msg.to_uid ) !== -1 ) {
				logger.debug( `[transport/command] Msg #${ msg.msgId } command ignored (in disables).` );
				return Promise.resolve();
			}
			...
			if ( !enables || ( enables && enables.indexOf( msg.to_uid ) !== -1 ) ) {
				...
				logger.debug( `[transport/command] Msg #${ msg.msgId } command ignored (not in enables).` );
			}

			if ( func && ( typeof func === 'function' ) ) {
				logger.debug( `[transport/command] Msg #${ msg.msgId } command: ${ msg.command }` );
				return func( msg );
			} else {
				return Promise.resolve();
			}
