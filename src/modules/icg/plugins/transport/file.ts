...
import { file as fileTS } from 'icg/handlers/Context';
import { version, repository } from 'src/../package.json';
		...
		if ( context.extra.clients > 1 && context.extra.files && servemedia.type && servemedia.type !== 'none' ) {
			...
			for ( const [ index, file ] of context.extra.files.entries() ) {
				if ( servemedia.sizeLimit && servemedia.sizeLimit > 0 &&
					...
					logger.debug( `[file.js] <FileUploader> #${ context.msgId } File ${ index + 1 }/${ fileCount }: Size limit exceeded. Ignore.` );
				} else {
			...
			for ( const [ index, upload ] of uploads.entries() ) {
				logger.debug( `[file.js] <FileUploader> #${ context.msgId } File ${ index + 1 }/${ uploads.length } (${ upload.type }): ${ upload.url }` );
			}
