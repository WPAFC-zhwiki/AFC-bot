...
export async function emitHook( event: string, msg: BridgeMsg ): Promise<void> {
	if ( hooks[ event ] ) {
		logger.debug( `[transport/bridge.js] emit hook: ${ event }` );
...
export async function send( m: BridgeMsg | Context ): Promise<boolean> {
	...
	logger.debug( `[transport/bridge.js] <UserSend> #${ currMsgId } ${ msg.from_uid } ---> ${ msg.to_uid }: ${ msg.text }` );
	const extraJson: string = JSON.stringify( msg.extra );
	if ( extraJson !== 'null' && extraJson !== '{}' ) {
		logger.debug( `[transport/bridge.js] <UserSend> #${ currMsgId } extra: ${ extraJson }` );
	}
	...
	for ( const t of msg.extra.mapto ) {
		...
		promises.push( emitHook( 'bridge.receive', msg2 ).then( function () {
			...
			if ( processor ) {
				logger.debug( `[transport/bridge.js] <BotTransport> #${ currMsgId } ---> ${ new_uid.uid }` );
				return processor( msg2, noPrefix );
			} else {
				logger.debug( `[transport/bridge.js] <BotTransport> #${ currMsgId } -X-> ${ new_uid.uid }: No processor` );
			}
		} ) );
	}
	try {
		...
		allresolved = false;
		logger.error( '[transport/bridge.js] <BotSend> Rejected: ', e );
	}
	emitHook( 'bridge.sent', msg );
	if ( promises.length > 0 ) {
		logger.debug( `[transport/bridge.js] <BotSend> #${ currMsgId } done.` );
	} else {
		logger.debug( `[transport/bridge.js] <BotSend> #${ currMsgId } has no targets. Ignored.` );
	}
