import { MessageHandler, Command } from 'icg/lib/handlers/MessageHandler';
...
import { getFriendlySize, getFriendlyLocation, copyObject } from 'icg/lib/util';

import { ConfigTS } from 'src/util/config';

import { tgBot as client } from 'src/util/bots';
...
type SendMessageOpipons = TT.ExtraEditMessage & TT.ExtraReplyMessage & {
	withNick?: boolean
};
		...
		client.on( 'message', async function ( ctx, next ) {
			if ( that._enabled && ctx.message && ctx.chat ) {
				...
				if ( ctx.message.reply_to_message ) {
					const reply: Partial<TT.Message> = ctx.message.reply_to_message;
