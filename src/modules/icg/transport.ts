...
export { BridgeMsg } from 'icg/transport/BridgeMsg';

import * as bridge from 'icg/transport/bridge';
export * from 'icg/transport/bridge';
export * from 'icg/transport/command';
import 'icg/transport/file';
import 'icg/transport/paeeye';
...
// 调试日志
logger.debug( '' );
logger.debug( '[transport] Bridge Map:' );
...
// 调试日志
logger.debug( '' );
logger.debug( '[transport] Aliases:' );
let aliasesCount = 0;
...
for ( const [ type ] of Manager.handlers ) {
	...
	logger.debug( `[transport] load processor ${ type }` );
