...
const USERAGENT = 'LilyWhiteBot/1.6.0alpha (https://github.com/lziad/LilyWhiteBot)';
		...
		if ( context.extra.clients > 1 && context.extra.files && servemedia.type && servemedia.type !== 'none' ) {
			...
			for ( let [ index, file ] of context.extra.files.entries() ) {
				if ( servemedia.sizeLimit && servemedia.sizeLimit > 0 && file.size && file.size > servemedia.sizeLimit * 1024 ) {
					logger.info( `\x1b[33m[Connect]\x1b[0m [file.js] <FileUploader> #${ context.msgId } File ${ index + 1 }/${ fileCount }: Size limit exceeded. Ignore.` );
				} else {
			...
			for ( let [ index, upload ] of uploads.entries() ) {
				logger.info( `\x1b[33m[Connect]\x1b[0m [file.js] <FileUploader> #${ context.msgId } File ${ index + 1 }/${ uploads.length } (${ upload.type }): ${ upload.url }` );
			}
...
module.exports = ( bridge, options ) => {
	...
	bridge.addHook( 'bridge.send', async ( msg ) => {
		try {
			...
			logger.info( '\x1b[33m[Connect]\x1b[0m Error on processing files: ', e );
