  ...
  const hook = ( event ) => ( msg ) => {
    if ( msg.command ) {
      ...
      if ( disables.indexOf( msg.to_uid ) !== -1 ) {
        logger.info( `\x1b[33m[Connect]\x1b[0m [command.js] Msg #${ msg.msgId } command ignored.` );
        return Promise.resolve();
      }
      ...
      if ( func && ( typeof func === 'function' ) ) {
        logger.info( `\x1b[33m[Connect]\x1b[0m [command.js] Msg #${ msg.msgId } command: ${ msg.command }` );
        return func( msg );
      } else {
        return Promise.resolve();
      }
