...
const init = ( b, h, c ) => {
  ...
  discordHandler.on( 'text', ( context ) => {
    const send = () => bridge.send( context ).catch( ( e ) => logger.info( `\x1b[33m[Connect]\x1b[0m ${ e.trace }` ) );
    ...
    if ( /<@\d*?>/u.test( context.text ) ) {
      ...
      for ( let at of ats ) {
        if ( userInfo.has( at ) ) {
          ...
          promises.push( discordHandler.fetchUser( at ).catch( ( e ) => logger.info( `\x1b[33m[Connect]\x1b[0m ${ e.stack }` ) ) );
        }
      }
      ...
      } ).catch( ( e ) => logger.info( `\x1b[33m[Connect]\x1b[0m ${ e.trace }` ) ).then( () => send() );
    } else {
      send();
    }
  } );
