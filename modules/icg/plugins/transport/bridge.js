...
const bridge = {
  ...
  send( m ) {
    ...
    logger.info( `\x1b[33m[Connect]\x1b[0m [bridge.js] <UserSend> #${ currMsgId } ${ msg.from_uid } ---> ${ msg.to_uid }: ${ msg.text }` );
    let extraJson = JSON.stringify( msg.extra );
    if ( extraJson !== 'null' && extraJson !== '{}' ) {
      logger.info( `\x1b[33m[Connect]\x1b[0m [bridge.js] <UserSend> #${ currMsgId } extra: ${ extraJson }` );
    }
    ...
    return prepareMsg( msg ).then( () => {
      ...
      for ( let t of msg.extra.mapto ) {
        ...
        promises.push( bridge.emitHook( 'bridge.receive', msg2 ).then( ( _ ) => {
          ...
          if ( processor ) {
            logger.info( `\x1b[33m[Connect]\x1b[0m [bridge.js] <BotTransport> #${ currMsgId } ---> ${ new_uid.uid }` );
            return processor.receive( msg2 );
          } else {
            logger.info( `\x1b[33m[Connect]\x1b[0m [bridge.js] <BotTransport> #${ currMsgId } -X-> ${ new_uid.uid }: No processor` );
          }
        } ) );
      }
      ...
      return Promise.all( promises )
        .catch( ( e ) => {
          allresolved = false;
          logger.info( '\x1b[33m[Connect]\x1b[0m [bridge.js] <BotSend> Rejected: ', e );
        } ).then( () => {
          bridge.emitHook( 'bridge.sent', msg );
          if ( promises.length > 0 ) {
            logger.info( `\x1b[33m[Connect]\x1b[0m [bridge.js] <BotSend> #${ currMsgId } done.` );
          } else {
            logger.info( `[bridge.js] <BotSend> #${ currMsgId } has no targets. Ignored.` );
          }
