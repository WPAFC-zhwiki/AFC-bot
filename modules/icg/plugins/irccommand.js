      ...
      if ( context.param ) {
        ...
        for ( let c of context.extra.mapto ) {
          ...
          if ( client.client === 'IRC' ) {
            ...
            logger.info( `\x1b[33m[Connect]\x1b[0m [irccommand.js] Msg #${ context.msgId }: IRC command has sent to ${ client.id }. Param = ${ context.param }` );
          }
        }

        if ( sentCount === 0 ) {
          logger.info( `\x1b[33m[Connect]\x1b[0m [irccommand.js] Msg #${ context.msgId }: No IRC targets.` );
        }
      } else {
