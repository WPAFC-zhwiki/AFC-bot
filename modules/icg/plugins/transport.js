...
module.exports = ( pluginManager, options ) => {
	...
	for ( let group of groups ) {
		...
		if ( group instanceof Array ) {
			...
			// 舊版
			logger.warn( '* Deprecated: config.example.json changed, please update "groups" in your config.js.' );
	...
	// 调试日志
	logger.info( '\x1b[33m[Connect]\x1b[0m [transport.js] Bridge Map:' );
	for ( let client1 in map ) {
		for ( let client2 in map[ client1 ] ) {
			if ( map[ client1 ][ client2 ].disabled ) {
				logger.log( `${ client1 } -X-> ${ client2 }` );
			} else {
				logger.log( `${ client1 } ---> ${ client2 }` );
			}
		}
	}
	...
	// 调试日志
	logger.info( '\x1b[33m[Connect]\x1b[0m [transport.js] Aliases:' );
	let aliasesCount = 0;
	for ( let alias in aliases2 ) {
		logger.info( `\x1b[33m[Connect]\x1b[0m ${ alias }: ${ aliases2[ alias ].shortname } ---> ${ aliases2[ alias ].fullname }` );
		aliasesCount++;
	}
	if ( aliasesCount === 0 ) {
		logger.info( '\x1b[33m[Connect]\x1b[0m None' );
	}
