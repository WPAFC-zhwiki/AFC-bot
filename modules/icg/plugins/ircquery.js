...
const processWhois = ( context ) => {
	if ( context.param ) {
		ircHandler.whois( context.param ).then( ( info ) => {
			...
			logger.info( `\x1b[33m[Connect]\x1b[0m [ircquery.js] Msg #${ context.msgId } whois: ${ outputStr }` );
...
const processNames = ( context ) => {
	...
	for ( let chan of chans ) {
		...
		context.reply( outputStr );
		logger.info( `\x1b[33m[Connect]\x1b[0m [ircquery.js] Msg #${ context.msgId } names: ${ outputStr }` );
	}
};
...
const processTopic = ( context ) => {
	...
	for ( let chan of chans ) {
		...
		if ( topic ) {
			...
			logger.info( `\x1b[33m[Connect]\x1b[0m [ircquery.js] Msg #${ context.msgId } topic: ${ topic }` );
		} else {
			context.reply( `No topic for ${ chan }` );
			logger.info( `\x1b[33m[Connect]\x1b[0m [ircquery.js] Msg #${ context.msgId } topic: No topic` );
		}
	}
};
